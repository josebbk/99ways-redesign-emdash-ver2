...
**This section is currently broken on the live build — it's rendering EmDash's default placeholder content instead of the real posts below. Fixing that is a required deliverable of the next build pass, not optional.** The 3 posts listed here must be the actual rendered content before this section is considered done — verify the placeholder text ("Welcome — Your first post...") is gone, not just that a grid exists.

- Section title (`--text-h2`): exact text above, verbatim.
- **Layout:** 3-column card grid (desktop), 1-column stacked (mobile). Show the latest 3 posts (not all 6) — **confirmed, consistent with the compact-layout direction elsewhere in this spec.** Content collection should be sortable by date so this stays current automatically.
